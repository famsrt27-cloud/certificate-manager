...
export function SearchablePublicSearchPanel() {
  ...
  const clearSearchOutcome = () => {
    setState("idle"); setItems([]); setDownloads({}); setFormError(false); searchExpired.current = false;
  };
  const clearContextSearch = () => {
    setRecipientName(""); setProjectInput(""); setProjectName(null); setTrainingInput(""); setTrainingName(null);
    clearSearchOutcome();
  };
  const clearCertificateSearch = () => { setCertificateNumber(""); clearSearchOutcome(); };
  ...
  return <div className="mt-1 text-left">
    <div className="grid grid-cols-2 rounded-xl bg-[#edf1ed] p-1" role="tablist" aria-label="วิธีตรวจสอบใบประกาศ">
      <button type="button" role="tab" aria-selected={mode === "search"} onClick={() => { setMode("search"); setFormError(false); }} className={`min-h-11 rounded-lg px-3 text-sm font-semibold ${mode === "search" ? "bg-[#fffdf8] text-[#174f50] shadow-sm" : "text-[#657174]"}`}>ค้นหาใบประกาศ</button>
      <button type="button" role="tab" aria-selected={mode === "qr"} onClick={() => { setMode("qr"); setFormError(false); }} className={`min-h-11 rounded-lg px-3 text-sm font-semibold ${mode === "qr" ? "bg-[#fffdf8] text-[#174f50] shadow-sm" : "text-[#657174]"}`}>ตรวจสอบด้วย QR</button>
    </div>
    {mode === "qr" ? <div className="py-10 text-center"><svg className="mx-auto size-20 text-[#174f50]" viewBox="0 0 48 48" fill="none" aria-hidden="true"><path d="M8 8h12v12H8V8Zm20 0h12v12H28V8ZM8 28h12v12H8V28Zm22 0h4v4h-4v-4Zm6 0h4v8h-4v-8Zm-6 8h4v4h-4v-4Zm6 2h4v2h-4v-2Z" stroke="currentColor" strokeWidth="2" /></svg><h1 id="verification-title" className="mt-5 text-2xl font-semibold text-[#172b2d] sm:text-3xl">ตรวจสอบด้วย QR</h1><p className="mx-auto mt-4 max-w-sm leading-7 text-[#667176]">สแกน QR บนใบประกาศด้วยกล้องของอุปกรณ์ แล้วระบบจะเปิดผลการตรวจสอบทันที</p></div> : <div className="pt-7">
      ...
      <form className="mt-6 space-y-4" onSubmit={(event) => void search(event)} noValidate>
        {exactMode ? <div className="rounded-xl border border-[#d7ded9] bg-[#f4f6f3] px-4 py-3 text-sm leading-6 text-[#52615d]" role="status">กำลังค้นหาด้วยเลขที่ใบประกาศ <button className="ml-1 font-semibold text-[#174f50] underline underline-offset-2" onClick={clearCertificateSearch} type="button">ล้างเลขที่เพื่อค้นหาด้วยชื่อ</button></div> : null}
        <div><label className="mb-1.5 block text-sm font-semibold text-[#34494b]" htmlFor="recipient-name">ชื่อผู้รับใบประกาศ</label><input id="recipient-name" value={recipientName} onChange={(event) => { setRecipientName(event.target.value); clearSearchOutcome(); }} autoComplete="name" maxLength={200} disabled={exactMode} aria-invalid={formError || undefined} aria-describedby={formError ? "public-search-error" : "recipient-name-hint"} className="min-h-12 w-full rounded-xl border border-[#c9d0cc] bg-white px-4 outline-none transition focus:border-[#427d7d] focus:ring-3 focus:ring-[#b8d5ce]/50 disabled:bg-[#f0f1ee]" /><p className="mt-1.5 text-xs text-[#75807c]" id="recipient-name-hint">กรอกชื่อด้วยตนเอง ระบบจะไม่แนะนำชื่อผู้รับ</p></div>
        <SearchableCombobox kind="project" label="โครงการ" placeholder="พิมพ์ชื่อโครงการ" disabled={exactMode}
          ...
          onChange={(value) => { setProjectInput(value); setProjectName(null); setTrainingInput(""); setTrainingName(null); clearSearchOutcome(); }}
          onSelect={(value) => { setProjectInput(value); setProjectName(value); setTrainingInput(""); setTrainingName(null); clearSearchOutcome(); }} />
        <SearchableCombobox kind="training" label="การอบรม" placeholder="พิมพ์ชื่อการอบรม" projectName={projectName}
          ...
          onChange={(value) => { setTrainingInput(value); setTrainingName(null); clearSearchOutcome(); }}
          onSelect={(value) => { setTrainingInput(value); setTrainingName(value); clearSearchOutcome(); }} />
        ...
        <div><label className="mb-1.5 block text-sm font-semibold text-[#34494b]" htmlFor="certificate-number">เลขที่ใบประกาศ</label><input id="certificate-number" value={certificateNumber} onChange={(event) => { setCertificateNumber(event.target.value); clearSearchOutcome(); }} autoComplete="off" maxLength={200} disabled={contextMode} aria-invalid={formError || undefined} aria-describedby={formError ? "public-search-error" : contextMode ? "certificate-number-disabled-hint" : undefined} className="min-h-12 w-full rounded-xl border border-[#c9d0cc] bg-white px-4 font-medium outline-none focus:border-[#427d7d] focus:ring-3 focus:ring-[#b8d5ce]/50 disabled:bg-[#f0f1ee]" />{contextMode ? <p className="mt-1.5 text-xs leading-5 text-[#75807c]" id="certificate-number-disabled-hint">กำลังค้นหาด้วยชื่อและบริบท <button className="font-semibold text-[#174f50] underline underline-offset-2" onClick={clearContextSearch} type="button">ล้างข้อมูลเพื่อใช้เลขที่ใบประกาศ</button></p> : null}</div>
        {formError ? <p className="rounded-lg border border-[#e5c6bf] bg-[#fbf1ee] px-3 py-2 text-sm text-[#7c342d]" id="public-search-error" role="alert">กรุณากรอกเลขที่ใบประกาศ หรือกรอกชื่อผู้รับและเลือกโครงการหรือการอบรม</p> : null}
