...
import {
  CreateTemplateRequestSchema, DeleteDraftVersionResponseSchema, TemplateAssetListResponseSchema, TemplateAssetResponseSchema,
...
export function TemplateWorkspace({ csrfToken, membership, templateId }: { readonly csrfToken: string; readonly membership: Membership; readonly templateId: string }) {
  ...
  const requestVersion = (id: string) => { if (id === selectedVersionId) return; if (dirty) { setDiscardTarget({ type: "version", id }); return; } applyVersion(versions.find((version) => version.id === id)); };
  ...
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if (isTextEditingTarget(event.target) || assetPickerKind !== null || uploadOpen || renameOpen || confirm !== null
