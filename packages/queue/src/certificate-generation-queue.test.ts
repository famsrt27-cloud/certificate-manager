...
import {
  ...
  CertificateGenerationJobPayloadSchema,
  createCertificateGenerationWorker
} from "./certificate-generation-queue.js";
...
describe("certificate generation queue contract", () => {
  ...
  it("rejects unsafe worker concurrency before opening BullMQ", () => {
    expect(() => createCertificateGenerationWorker({ connection: {} as never, prefix: "test", concurrency: 0,
      process: async () => undefined, onFinalFailure: async () => undefined })).toThrow("concurrency is invalid");
  });
});
